...
/**
 * AI 해석 관점 키워드 → 프롬프트용 풍부한 설명 매핑.
 * TrainerApp 의 perspectiveChip(string) 값이 그대로 buildSessionLogPrompt 의
 * perspective 로 전달됨 → 여기서 매핑된 텍스트를 프롬프트에 박아 일관성·전문성↑.
 * 매핑에 없는 자유 텍스트가 와도 그대로 사용 (사용자 직접 작성 호환).
 */
const PERSPECTIVE_TEXT = {
  rehab:       '재활·부상 케어 관점: 통증·가동범위·보상 패턴·관절 부담을 우선 관찰. 자극은 점진적, 회복·정렬을 운동 강도보다 앞에 둠. 무리·통증 언급 시 즉시 강도 조절·대체 동작 제안. 강제적 한계 도전 금지.',
  motivation:  '동기부여·심리 관점: 회원의 감정·자신감·지속 의지에 주목. 작은 성취(이전 대비 향상·정확한 자세·꾸준한 출석)를 구체적으로 짚어 칭찬. 비교·평가보다 격려·기록·다음 한 걸음 강조.',
  performance: '퍼포먼스·기술 관점: 동작 효율·근신경 활성도·중량 진척·RIR 정확도에 주목. 자세 디테일(중심점·관절 각도·호흡 타이밍)을 짧고 정확하게 코칭. 데이터(중량·RIR·세트별 감각) 기반으로 다음 진척 방향 제시.',
  diet:        '다이어트·체성분 관점: 운동 자극과 식단·회복·체구성 변화 연관 짚기. 칼로리 소모·근육 보존·단백질 회복 관점에서 오늘 수업 의미 해석. 운동 후 식사·수분·수면 같은 회복 행동 자연스레 안내.',
  ak:          '응용근신경학(AK) 관점: 근육 약화를 단순 강화 대상이 아닌 신경학적 신호로 봄(반사 억제·관절 기능부전·구조적 약점). 약화된 근육·과활성 보상 패턴을 식별해 원인 가설(신경·관절·내장·정서 연관) 짧게 언급. 단순 "더 무겁게/더 많이"가 아니라 신경학적 정상화 방향 제시.',
  complex:     '복잡계 관점: 신체를 비선형 적응 시스템으로 봄. 자극→반응이 1:1 비례가 아님. 작은 변화의 비례 안 되는 효과, 패턴 변화·창발·자기조직화 강조. "근육별 분리 처방"보다 "전체 시스템 적응" — 컨디션 변동·과거 자극 누적·환경(수면·스트레스) 영향 짚기.',
}
 ...
 *
 * 핵심 규칙 (2026-05-21 강화):
 *   - 트레이너 입력(메모/음성)에 있는 내용만 정리·구조화·재해석.
 *   - 입력에 없는 운동·세트·증상·일반론(교과서적 조언) 임의 추가 금지.
 *   - 빈 섹션은 출력 자체를 생략 (방대화·과장 차단).
 *   - 해석 관점은 PERSPECTIVE_TEXT 매핑된 풍부한 설명을 박아 일관성·전문성 확보.
 *
 * @param {object} p
 ...
 * @param {string} [p.rawInput]       - 트레이너 메모 (STT 또는 직접 입력)
 * @param {string} [p.perspective]    - AI 해석 관점 키워드(rehab/motivation/performance/diet/ak/complex) 또는 자유 텍스트
 * @param {boolean} [p.hasAudio]      - 오디오 파일 포함 여부 (멀티파트 시 true)
 * @returns {string} Gemini 프롬프트
 */
export function buildSessionLogPrompt({
  trainer, member, exercises = [], rawInput = '',
  perspective = '', hasAudio = false,
}) {
  ...
  // 매핑된 키워드면 풍부한 설명, 매핑 없으면 입력 텍스트 그대로 (자유 작성 호환)
  const pKey  = (perspective || '').trim()
  const pText = PERSPECTIVE_TEXT[pKey] || pKey
  const perspectiveInstruction = pText
    ? `\n⭐ 해석 관점 (이 관점을 중심으로 — 단 트레이너 입력 범위 안에서만 적용):\n${pText}\n`
    : ''

  const rule1 = hasAudio
    ? '음성에 수업과 무관한 사적 대화가 포함될 수 있습니다 — 완전히 무시하세요.'
    : '트레이너의 투박한 브리핑 메모를 다듬되, 없는 내용은 만들지 마세요.'

  const memoLabel = hasAudio ? '[추가 메모]' : '[수업 브리핑 메모]'

  return `${audioInstruction}당신은 전문 퍼스널 트레이너의 수업일지 작성 도우미입니다.

⚠️ 핵심 작성 규칙 (반드시 준수):
1. 트레이너 입력(메모/음성)에 있는 내용만 정리·구조화·재해석하세요. 입력에 없는 운동·세트·증상·일반론을 임의로 추가하지 마세요.
2. 빈 섹션은 출력하지 마세요. 예: 트레이너가 "다음 목표"를 안 적었으면 🎯 섹션 자체를 생략.
3. 일반론·교과서적 조언("물을 충분히 마시세요", "단백질 섭취 권장" 등)은 입력에 없으면 넣지 마세요.
4. ${rule1}
5. 세트별 RIR·감각 피드백은 입력에 있을 때만 표기. 없는 값을 추측해 채우지 마세요.
6. 친근하고 전문적인 톤, 적절한 이모지 사용, 중복 제거.
...
다음 형식으로 작성하되, 입력에 해당 내용이 없으면 그 섹션 자체를 생략하세요:

📋 수업일지 - ${member.name} 회원님
📅 ${today} | ${session}/${member.total_sessions}회차

🏋️ 오늘의 운동
[입력된 운동·세트만. 입력 없으면 섹션 생략]

💬 트레이너 코멘트
[입력에 명시된 피드백·관찰만 재해석. 추측·일반론 금지. 입력 없으면 섹션 생략]

🎯 다음 수업 목표
[입력에 명시된 목표만. 입력 없으면 섹션 생략]
