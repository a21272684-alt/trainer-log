...
--
-- ⚠️ 주의 — admin 영역(`admin.html`)이 anon 으로 trainers/members 직접
--   read 하던 부분은 이 마이그레이션 후 차단됨. admin 작업은
--   Supabase Dashboard SQL Editor 에서 직접 실행하거나 별도 admin RPC
--   도입 후 복구 예정. 사용자가 잠깐 admin 사용 보류 필요.
...
CREATE POLICY "members_self_or_trainer_select" ON members
  ...
  USING (
    ...
    OR trainer_id IN (
      SELECT id FROM trainers WHERE auth_id = auth.uid()::text
    )
  );
...
--         (c) 매칭 X + p_name/p_phone 제공 → 신규 INSERT (등록 화면 완료 후)
...
BEGIN
  ...
  -- (a) auth_id 매칭
  SELECT * INTO v_row FROM trainers WHERE auth_id = v_uid_t LIMIT 1;
  IF FOUND THEN RETURN v_row; END IF;

  -- (b) email 매칭 → auth_id 채움
  IF p_email IS NOT NULL AND p_email <> '' THEN
    UPDATE trainers SET auth_id = v_uid_t
  ...
  IF p_name IS NOT NULL AND p_name <> '' THEN
    INSERT INTO trainers (name, phone, email, auth_id)
      VALUES (p_name, COALESCE(p_phone, ''), p_email, v_uid_t)
      RETURNING * INTO v_row;
    RETURN v_row;
  END IF;
...
BEGIN
  ...
  -- (a) auth_id 매칭
  SELECT * INTO v_row FROM members WHERE auth_id = v_uid_t LIMIT 1;
  IF FOUND THEN RETURN v_row; END IF;

  -- (b) email 매칭 → auth_id 채움
  IF p_email IS NOT NULL AND p_email <> '' THEN
    UPDATE members SET auth_id = v_uid_t
      WHERE email = p_email AND auth_id IS NULL
      RETURNING * INTO v_row;
    IF FOUND THEN RETURN v_row; END IF;
  END IF;
