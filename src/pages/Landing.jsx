...
const FAQS = [
  {
    ...
    a: '아니요. 무료 플랜에서도 월 20회의 AI 수업일지를 사용할 수 있어요. 별도 결제 수단 등록이 필요 없고, 한도를 넘으면 자동으로 멈출 뿐 추가 요금은 발생하지 않아요.',
  },
  {
    ...
    <div style={{background:'#f8fafc',color:'#0f172a',minHeight:'100vh',fontFamily:"'Noto Sans KR',sans-serif",overflowX:'hidden'}}>
        ...
        <div style={{maxWidth:'860px',margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'24px',textAlign:'center'}}>
          {stats.map((s,i) => (
            <div key={i} style={{padding:'8px'}}>
              <div style={{fontSize:'clamp(28px,5vw,42px)',fontWeight:900,letterSpacing:'-2px',color:'#0f172a',lineHeight:1}}>{s.num}</div>
      ...
      <section style={{background:'#f8fafc',padding:'80px 24px'}}>
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'16px'}}>
            {PROBLEMS.map((p,i) => (
              <div key={i} style={{background:'#fff',border:'1px solid #fee2e2',borderRadius:'16px',padding:'24px',
      ...
      <section style={{background:'#fff',padding:'80px 24px'}}>
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'16px'}}>
            {SOLUTIONS.map((s,i) => (
              <div key={i} style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:'16px',padding:'24px',
      ...
      <section style={{background:'#fff',padding:'80px 24px'}}>
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'14px'}}>
            {kakao.map((msg, i) => (
              <div key={i} style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:'16px',padding:'16px',
      ...
      <section style={{background:'#f8fafc',padding:'80px 24px'}}>
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          <div style={{textAlign:'center',marginBottom:'48px'}}>
            <div style={{fontSize:'11px',fontWeight:700,letterSpacing:'0.13em',color:'#7c3aed',marginBottom:'10px'}}>REVIEWS</div>
            <h2 style={{fontSize:'clamp(22px,4vw,32px)',fontWeight:800,color:'#0f172a',letterSpacing:'-1px',margin:'0 0 10px',lineHeight:1.3}}>
              실제 트레이너들의 이야기
            </h2>
      ...
      <section style={{background:'#fff',padding:'80px 24px'}}>
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:'16px'}}>
            {TARGETS.map((t,i) => (
              <div key={i} style={{background:t.bg,border:`1px solid ${t.border}`,borderRadius:'16px',padding:'24px'}}>
