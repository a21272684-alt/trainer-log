...
// 로컬 참조용 import (re-export만으로는 같은 파일 내에서 호출 불가)
import {
  buildGymWeeklyReportPrompt,
  callGemini,
} from './ai_templates'
...
export async function generateWeeklyReport({
  ...
  try {
    ...
    const prompt = buildGymWeeklyReportPrompt(stats)

    // ④ Gemini 호출 (ai_templates.callGemini)
    onStatus('🤖 AI 리포트 생성 중...')
    const reportText = await callGemini(apiKey, model, prompt)
